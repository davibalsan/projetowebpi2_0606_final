...
async function carregaDadosAlteracao(db, id) {
  await firebase.database().ref(db + '/' + id).on('value', (snapshot) => {
    ...
    if (snapshot.val().congelado === 'SIM') {
      document.getElementById('SIM').checked = true
    } else {
      document.getElementById('NÃO').checked = true
    }
  })
...
function salvar(event, collection) {
  ...
  else if (document.getElementById('numeronotafiscal').value === '') { alerta('⚠️É obrigatório informar o número da nota fiscal!', 'warning') }
  else if (document.getElementById('datacompra').value === '') { alerta('⚠️É obrigatório informar a data da compra!', 'warning') }
  else if (document.getElementById('preçocompra').value < 0 || document.getElementById('preçocompra').value > 300) { alerta('⚠️O peso deve ser um número entre 0 a 300', 'warning') }
